...
- 강조색은 **초록**입니다. `--accent`(#15803d)는 흰 글자를 얹는 상호작용용이라
  대비 4.5:1을 넘깁니다. `--accent-bright`(#22a55b)는 큰 숫자·아이콘 전용이고
  3:1 기준이므로 본문에는 쓰지 않습니다.
- 화면 제목은 28px/800, 소제목은 17px/700입니다. 통계 타일은 아이콘(연한 강조
  배경) → 큰 숫자 → 작은 라벨 순서입니다.
- 구간 선택은 세그먼트 컨트롤(`.segmented`)입니다. 개별 칩을 나열하지 않습니다.
- 목록은 `.record-row`(날짜 → 뱃지 → 요약 → 꺾쇠) 형태이고, 기본 6줄만 보이고
  나머지는 '더보기'로 펼칩니다.
